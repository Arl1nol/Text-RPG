...
class Boss:
    ...
    def is_dead(self, player):
        if self.hp < 1:
            self.is_alive = False
            shake_text(f"The {self.name_display} has been {Fore.GREEN}defeated{Style.RESET_ALL}!!!")
    ...
    def attack(self, target):
        time.sleep(0.5)
        typewriter(f"\nThe {self.name_display} {Fore.BLACK}{Style.BRIGHT}strikes{Style.RESET_ALL}!")
    ...
    def special_attack(self, p1):
        if self.name == 'Lich':
            ...
            if self.special_attack_counter == 0:
                typewriter(
                    f"{self.name_display}'s sword starts {Fore.MAGENTA}pulsating{Style.RESET_ALL} with dark energy.")
                self.special_attack_counter += 1
            elif self.special_attack_counter == 1:
                shake_text(
                    f"{self.name_display} raises his sword, the ground begins to crack!")
                self.special_attack_counter += 1
